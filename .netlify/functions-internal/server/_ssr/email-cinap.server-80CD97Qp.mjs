//#region node_modules/.nitro/vite/services/ssr/assets/email-cinap.server-80CD97Qp.js
var NAVAL = "#0F2240";
var DOURADO = "#D9A74A";
var AZUL_MEDIO = "#1A3A5C";
function escapar(texto) {
	return texto.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;");
}
/** Template institucional CINAP em HTML puro (compatível com clientes de e-mail). */
function montarEmailAviso(config, d) {
	const linhas = d.paragrafos.map((p) => `<p style="margin:0 0 14px 0;font-size:14px;line-height:22px;color:${NAVAL};">${escapar(p)}</p>`).join("");
	const bloco = d.valor ? `<table role="presentation" width="100%" style="border-collapse:collapse;margin:18px 0;">
        <tr>
...
async function enviarEmailCinap(config, para, assunto, conteudo, idempotencyKey) {
	const em = (/* @__PURE__ */ new Date()).toISOString();
	if (!config.emails_ativos) return {
		enviado: false,
		erro: "Alertas por e-mail desativados na configuração.",
		message_id: null,
		em
	};
	if (!config.remetente_email || !config.dominio_email) return {
		enviado: false,
		erro: "Remetente/domínio de e-mail não configurado.",
		message_id: null,
		em
	};
	...
	if (!apiKey || !apiUrl) return {
		enviado: false,
		erro: "Chave de API de e-mail indisponível no servidor.",
		message_id: null,
		em
	};
	try {
...
//#endregion
export { enviarEmailCinap, montarEmailAviso };
